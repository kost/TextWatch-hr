...
*/
  ...
  if (minutes == 0 ) {
	  if (hours == 0) {
	    ...
	    if (hours==1) {
	       remaining2 -= append_string(words, remaining, STR_OH_CLOCK);
	    } else if (hours<5) {
	       remaining2 -= append_string(words, remaining, STR_OH_MCLOCK);
	    } else {
	       remaining2 -= append_string(words, remaining, STR_OH_PCLOCK);
	    }
	  }
  } else {
    remaining -= append_number(words, hours % 12);
